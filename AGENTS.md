...
- `bun run ci` — Astro checking, scripts type checking, and production build; used locally and in PR CI.
...
- `immortality.yml` uses the automatic `GITHUB_TOKEN` with Actions write permission
  to keep this repository's active and inactivity-disabled workflows enabled.
  It preserves manually disabled workflows. `gh-workflow-immortality.sh` is retained
  vendored MIT third-party code (v1.1.1), but is not invoked by the workflow.
  Update that script wholesale from upstream, not by hand-editing. The repository
  LICENSE is AGPL-3.0; that script keeps its own MIT header.
- `renovate.json` inherits versioned same-account presets. The complete
  `ci / required` gate checks every prerequisite. Dependency service access and
  merge handling must be configured separately from the presets.
  TypeScript stays below 7 until the Astro checker supports its compiler API.
  See `docs/ci.md` for coverage and compatibility details.
